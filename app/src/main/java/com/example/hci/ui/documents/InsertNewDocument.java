...
public class InsertNewDocument extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        String selector = Objects.requireNonNull(bundle).getString("label");
        ...
        status.setOnSpinnerItemSelectedListener((parent, view, position, id) -> {
            if (position == 0) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == CAMERA_REQUEST && resultCode == RESULT_OK) {
            selImg = (Bitmap) Objects.requireNonNull(data.getExtras()).get("data");
            docImg.setImageBitmap(selImg);
        }
        else if (requestCode == PICK_IMAGE && resultCode == RESULT_OK) {
            try {
                final Uri imageUri = data.getData();
                final InputStream imageStream = requireActivity().getContentResolver()
                        .openInputStream(Objects.requireNonNull(imageUri));
